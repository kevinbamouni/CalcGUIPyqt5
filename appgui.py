...
##        (fileName, filtre) = QtWidgets.QFileDialog.getOpenFileName(self, "Open File", "", "CSV Files (*.csv)");

        df = pd.read_csv("data.csv")

#        Affichage tab 2
        m = pddataframemodel.pddataframemodel(df)
        self.tableView.setModel(m)

#       Affichage tab 1
        self.tableWidgetDataFrame(df, self.tableWidget)


    def tableWidgetDataFrame(self, df, tableWidget):
        # Affichage tab 1
        tableWidget.setRowCount(df.shape[0])
        tableWidget.setColumnCount(df.shape[1])
        tableWidget.setHorizontalHeaderLabels(df.columns)
        # Disable sorting before printing data in the view and enable it then
        tableWidget.setSortingEnabled(False)
        tableWidget.setEnabled(True) # Activer/désactiver le tableWidget
        tableWidget.setEditTriggers(QtWidgets.QAbstractItemView.NoEditTriggers) # Desactive l'edition
        tableWidget.setStyleSheet("QTableView {selection-background-color: light-grey; color: blue}")


        for row in range(df.shape[0]):
            for col in range(df.shape[1]):
                x = df.iat[row,col]
                y = QtWidgets.QTableWidgetItem(str(x), 10001)

                tableWidget.setItem(row, col, y)
